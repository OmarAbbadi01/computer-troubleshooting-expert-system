...
```bash
python3 -m py_compile main.py engine.py questionnaire.py knowledge_base.py models.py bilingual.py evaluation/run_evaluation.py evaluation/cases.py
```
...
| `evaluation/` | 10 (+1) حالات تقييم؛ منفصلة عن التطبيق | للاختبار فقط |
...
الاستدلال، فهي حالات تحقق حقيقية لا دافعة للتطوير. التطبيق لا يحتوي أي
منطق خاص بسيناريوهات الاختبار ولا يشير إليها.
...
تُعرَّف السيناريوهات في `evaluation/cases.py` (للتقييم فقط) ويعمل المشغّل عبر
الاستبيان والمحرك الحقيقيين.

### 8.2 الحالات الإحدى عشرة
...
- **لا سلوك محفور:** تُشغَّل السيناريوهات عبر الاستبيان والمحرك العاديين؛
...
تشخيصات متعددة + أدلة غير كافية. تُخزَّن في `evaluation/cases.py` وتُشغَّل عبر
الاستبيان والمحرك الحقيقيين بالأمر `python3 -m evaluation.run_evaluation`.
التطبيق نفسه لا يعرف هذه الحالات.
...
| 0:15–0:16 | 5. الاختبار | جدول `run_evaluation.py` (اختياري عند ضيق الوقت) |
